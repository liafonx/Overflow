...
local function get_card_slot_cost(card, qty_override)
    if not card then
        return 0
    end
...
function Overflow.get_consumeable_count(include_buffer)
    if not G or not G.consumeables or not G.consumeables.cards then
        return 0
    end

    local total = 0
    if Overflow.should_enforce_consumeable_slots() then
        for _, v in ipairs(G.consumeables.cards) do
            total = to_big(total) + to_big(get_card_slot_cost(v))
        end
    else
...
function Overflow.can_accept_consumeable(card, incoming_qty, merge_target, include_buffer)
    if not Overflow.should_enforce_consumeable_slots() then
        return true
    end
    ...
    if merge_target then
        local before = get_card_slot_cost(merge_target)
        local after = get_card_slot_cost(merge_target, to_big(merge_target.qty or 1) + to_big(incoming))
        delta = to_big(after) - to_big(before)
    else
        delta = get_card_slot_cost(card, incoming)
    end
...
function Overflow.sync_consumeable_card_count()
    if G and G.consumeables and G.consumeables.config then
        G.consumeables.config.card_count = Overflow.get_consumeable_count(false)
    end
end
